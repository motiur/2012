...
def crawl_web(seed,max_depth):
    ...
    while tocrawl:
        ...
        page = tocrawl.pop()#This wil pop the bottom most element 

        if page not in crawled:#This page has not been added to crawled
            
            crawled.append(page)#Add the page first 

            for val in depth:

                if val[1] == max_depth: page = None#If you reach the max_depth
                                                    #do not need to crawl further
                
            union(tocrawl, get_all_links(get_page(page)))#Union of page=None adds
                                                        #nothing
            
            for links in get_all_links(get_page(page)):#If page!=None , get all the
                                                        #the links from the page
                                                        #and add them
                depth.append([links, i])
